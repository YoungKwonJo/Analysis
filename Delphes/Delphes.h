...
// Mon Aug 18 18:36:52 2014 by ROOT version 5.32/00
// from TTree Delphes/Analysis tree
// found on file: ../data/events_PYTHIA8_v20140813_ttbb_loop_sm_cut_bq.root
//////////////////////////////////////////////////////////
...
public :
   ...
   virtual int isFromAny(int idx, int x , int id);
   virtual int isFromTop(int idx, int x );
   virtual int isFromBQ(int idx, int x );
   virtual int isFromCQ(int idx, int x );
   virtual int isFromHiggs(int idx, int x );

   virtual bool HasAnyHadron(int PDGID, int id);
   virtual bool HasBHad(int PDGID);
   virtual bool HasCHad(int PDGID);
   virtual int isFromAnyHad(int idx, int x, int id);
   virtual int isFromBHad(int idx, int x);
   virtual int isFromCHad(int idx, int x);

/*
   virtual int isHiggsToBB(int idx); 
*/
   virtual GenParticle getGenParticle(int idx);
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("../data/events_PYTHIA8_v20140813_ttbb_loop_sm_cut_bq.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("../data/events_PYTHIA8_v20140813_ttbb_loop_sm_cut_bq.root");
      }
