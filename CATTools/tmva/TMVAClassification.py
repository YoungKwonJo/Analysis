...
DEFAULT_INFNAME  = "file:/cms/home/youngjo/CATools/ntuple_v741/result4/result_TT_powheg.root"
DEFAULT_TREESIG  = "myresult2"#"TreeS"
DEFAULT_TREEBKG  = "myresult2"#"TreeB"
...
DEFAULT_METHODS  = "CutsD,LikelihoodD,MLP,MLPBFGS,MLPBNN,BDT,BDTD,BDTG"
...
def main():
    ...
    factory.AddVariable( "jet_csv[0]",   "jet_csv[0]", "units", 'F' )
    factory.AddVariable( "jet_csv[1]",   "jet_csv[1]", "units", 'F' )
    factory.AddVariable( "jet_csv[2]",   "jet_csv[2]", "units", 'F' )
    factory.AddVariable( "jet_csv[3]",   "jet_csv[3]", "units", 'F' )
    ...
    if gSystem.AccessPathName( infname ) != 0:
        print "can't open the "+str(infname)
        sys.exit(1)     
    ...
    from cut_cfi import mm_cuts,ee_cuts,em_cuts
    eventselection = "("+mm_cuts["cut"] +"||"+ee_cuts["cut"] +"||"+em_cuts["cut"]+")"
    def mAND(aaa,bbb):
        return "(" +aaa+ " && "+bbb+")"
    ...
    mycutSig = TCut( eventselection+"&&"+ttbb ) 
    mycutBkg = TCut( eventselection+"&&"+op_(ttbb) ) 
